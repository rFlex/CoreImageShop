...
{
    SCPlayer *player = (__bridge SCPlayer *)(displayLinkContext);
    
    CFTimeInterval nextVSync = inOutputTime->videoTime;
    NSLog(@"Next VSync: %f", nextVSync);
//    CFTimeInterval nextVSync = ([sender timestamp] + [sender duration]);
    
	CMTime outputItemTime = [player->_videoOutput itemTimeForHostTime:nextVSync];
    
	if ([player->_videoOutput hasNewPixelBufferForItemTime:outputItemTime]) {
		CVPixelBufferRef pixelBuffer = [player->_videoOutput copyPixelBufferForItemTime:outputItemTime itemTimeForDisplay:nil];
        
        if (pixelBuffer) {
            CVPixelBufferLockBaseAddress(pixelBuffer, 0);
            CIImage *image = [player->_filterGroup imageByProcessingImage:[CIImage imageWithCVImageBuffer:pixelBuffer]];
            player->_imageView.image = image;
            player->_imageView.hidden = NO;
